...
variable for bank balance sheets. This mapping is an analytic isomorphism,
...
Empirically, $U(t)$ is instantiated as a scalar credit-capacity gauge
constructed from the credit table (nominal credit or GDP-like quantities).
For JP, $U(t)$ is taken from a GDP-like ``energy'' series
($U_{\text{energy}}$, derived from nominal GDP) and falls back to nominal
GDP $Y(t)$ when that series is unavailable.
For EU and US, $U(t)$ is set equal to nominal GDP $Y(t)$ by default.
In all regions, if neither an explicit $U$ nor a $Y$ column is present,
the implementation falls back to real credit stocks $L_{\mathrm{real}}(t)$
as a last resort so that the first-law decomposition remains defined.
An optional detrended version $U^\star$ is produced by subtracting a
no-lookahead rolling mean; this $U^\star$ is used only in diagnostics and
visualisation (to study fluctuations around a slowly varying baseline),
while the core potentials $\FC$ and $\XC$ are computed from the undetrended
$U$.
...
markets are deep, and turnover is brisk, and to fall sharply when any one
...
point rather than a uniquely correct functional form.
...
area} traced out by trajectories in the $(\VC,\pC)$ plane over policy or
credit cycles.
...
paths in $(\SM,\VC)$ space exhibit pronounced hysteresis and stress.
...
\begin{enumerate}
  ...
  \item \textbf{Loop area and hysteresis.} Compute loop areas in $(\SM,\VC)$
...
\begin{itemize}
  ...
  \item \textbf{Capacity, headroom, and coupling.} The core constructor
        ...
        \texttt{lib/external\_coupling.py} into $E_p$ and $E_T$; when the
  ...
  \item \textbf{Diagnostics.}
        The diagnostics routine in \texttt{lib/indicators.py} estimates
        $\partial S_{\mathrm{M}}/\partial V_{\mathrm{C}}|_{T_{\mathrm{L}}}$
...
If trajectories in the $(\SM,\VC)$ plane form loops with non-zero area over
