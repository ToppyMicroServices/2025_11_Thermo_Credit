...
\date{Version 3.1 -- March 2026 (measurement-first and MCP distribution update)}

\begin{document}
\maketitle

\begin{abstract}
This note develops Thermo Credit as a measurement-first macro-credit model of
how the destination of new credit alters the transformation of credit expansion
into output, inflation, asset-price acceleration, and fragility. The framework
combines (i) an SFC-style conservation backbone based on balance-sheet
identities with (ii) a non-equilibrium ``credit thermodynamics'' layer in which
irreversibility and hysteresis are first-class objects. We introduce a minimal
state description built around credit partition, capacity, and stress,
including an internal bookkeeping potential $U(S_{\mathrm{M}},V_{\mathrm{C}},\xi,\ldots)$
with memory variable $\xi$, and we define operational gauges such as
Helmholtz-style free energy $F_{\mathrm{C}}$ and exergy-like availability
$X_{\mathrm{C}}$. Maxwell-like integrability is treated as a local-equilibrium
limit rather than a law; persistent violations are interpreted as measurable
non-equilibrium ``vorticity'' in the $(S_{\mathrm{M}},V_{\mathrm{C}})$ state
plane. The construction is deliberately practice-first: quantities are intended
to be computable from public data, judged by forecast usefulness and
falsifiability, and distributed in recalculable form through specification,
code, and AI-facing interfaces rather than through narrative alone.
\end{abstract}
...
The measurement-first revision narrows the framework to one research question:
how does the destination of new credit alter the transformation of credit
expansion into growth, inflation, asset-price acceleration, and fragility?
Operationally, the current version is designed for three immediate use cases:
\begin{enumerate}
  \item early-warning signals for asset inflation and systemic fragility;
  \item description of the real-versus-asset credit mix rather than credit
        volume alone;
  \item forecast improvement relative to total-credit-only baselines.
\end{enumerate}
This keeps the theory tied to a concrete mapping from inputs to outputs and
prevents the thermodynamic analogy from drifting into a purely descriptive
vocabulary.
...
explicit rejection conditions. If the partition variables $(C_t^R,C_t^A,q_t)$
and the stress state $S_t$ fail to improve empirical discrimination relative to
simpler credit-volume baselines, the measurement-first version of Thermo Credit
should be simplified or rejected in that application. A more systematic
...
outlined in \texttt{docs/thermo\_credit\_mcp\_spec.md}. Connection patterns for
local and remote AI clients are documented in
\texttt{docs/mcp\_connection\_guide.md}, while \texttt{llms.txt} provides a
short machine-facing reading order. Together these files distinguish directly
observed quantities, proxy-estimated quantities, and future latent-state
estimates, and they provide the shortest path from theory to recalculable
outputs.

\subsection{Publication stack and interface layers}

The repository now separates four publication layers:
\begin{enumerate}
  \item \texttt{report.html} for narrative interpretation and plots;
  \item \texttt{docs/definitions.md} and
        \texttt{data/data\_dictionary.csv} for fixed variable meanings and
        identification rules;
  \item \texttt{scripts/} and \texttt{tests/} for recalculation and empirical
        validation;
  \item an AI-facing resource/tool layer built from schemas, structured
        examples, and an MCP wrapper.
\end{enumerate}
This fourth layer should be understood as a \emph{distribution} mechanism
rather than a validation mechanism. It makes Thermo Credit easier for AI
systems to query and reuse, but it does not by itself establish the theory's
empirical correctness. Validation still depends on the forecasting tests,
identification strategy, and falsifiability criteria stated above.

\subsection{Canonical event windows}

To make chart reading and case-study writing more stable across the HTML
dashboard and this note, the repository now keeps a small shared event
registry in \texttt{data/report\_events.csv}. The intent is not to impose a
single historical taxonomy, but to keep major windows such as the IT bubble
unwind, the Lehman / global financial crisis, the euro sovereign debt crisis,
and the COVID-19 pandemic shock aligned across outputs. Dashboard charts shade
these windows whenever they overlap the visible sample, while this note can
quote the same registry directly.

\IfFileExists{generated/dashboard_events.tex}{
\paragraph{Canonical dashboard event registry.}
\input{generated/dashboard_events.tex}
}{%
\paragraph{Canonical dashboard event registry.}
Regenerate \texttt{site/report.html} to populate
\texttt{tex/generated/dashboard\_events.tex}.
}

\subsection{Paper-ready figures from dashboard data}
...
depend on browser screenshots or Plotly-specific rasterization. The aim is to
preserve consistency with the dashboard while using a cleaner visual grammar
...
\begin{figure}[htbp]
  ...
  \caption{Paper-ready regional panels for $S_M$ and $T_L$ rebuilt from the
  dashboard indicator tables. Each line uses a winsorized median/MAD score
  with smooth \texttt{asinh} compression within region, and the shaded windows reuse the shared event registry in
  \texttt{data/report\_events.csv}.}
\end{figure}
}{%
...
\begin{figure}[htbp]
  ...
  \caption{Paper-ready regional panels for $X_C$ and loop dissipation rebuilt
  from the dashboard indicator tables. Here $X_C$ first passes through a
  trailing four-quarter rolling median and a light two-quarter mean, after
  which the same winsorized median/MAD scaling with smooth \texttt{asinh}
  compression, event registry, and regional filtering rules are applied so
  that the paper layer and the dashboard remain aligned.}
\end{figure}
}{}

\subsection{Dashboard takeaways for citation}

The report build now also emits a small reuse layer for writing and slide
workflows: a LaTeX snippet in \texttt{tex/generated/dashboard\_takeaways.tex},
a machine-readable JSON payload in \texttt{site/dashboard\_takeaways.json}, and
a PNG summary card in \texttt{tex/generated/dashboard\_takeaways.png} when the
local image stack is available. This is intended to let the dashboard's latest
interpretation notes be quoted in papers and memos without copy-editing the
HTML by hand.

\IfFileExists{generated/dashboard_takeaways.tex}{
\paragraph{Auto-generated dashboard snapshot.}
\input{generated/dashboard_takeaways.tex}
}{%
\paragraph{Auto-generated dashboard snapshot.}
Regenerate \texttt{site/report.html} to populate
\texttt{tex/generated/dashboard\_takeaways.tex}.
}

\IfFileExists{generated/dashboard_takeaways.png}{
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{generated/dashboard_takeaways.png}
  \caption{Auto-generated dashboard takeaway card exported from the latest
  report build.}
\end{figure}
}{}
...
\begin{itemize}
  \item a Python pipeline computes $\SM$, $\TL$, $\VC$, $\FC$, $\XC$, and
        ...
        files and scripts;
  \item a measurement-first tool layer now exposes the compact model spec,
        variable definitions, core metric calculations, scenario evaluation,
        and regime comparisons through structured JSON interfaces;
...
\begin{itemize}
  ...
  \item \textbf{Measurement-first tool layer.}
        ...
        v2 interface used by the current repository-side AI layer:
        theory overview, variable definitions, metric computation,
        scenario evaluation, and regime comparison. The script
        \texttt{scripts/thermo\_credit\_cli.py} exposes the same layer as a
        JSON CLI for local reproducibility checks.
  \item \textbf{MCP distribution wrapper.}
        ...
        server entrypoint. This layer is designed for AI accessibility and
        structured reuse; it is not a substitute for the empirical validation
        logic discussed in the main text.
